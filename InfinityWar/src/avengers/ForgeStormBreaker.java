...
public class ForgeStormBreaker {
	public static void main (String [] args) {
        ...
        int[][] fluxNums = new int[numOfRows][numOfCols];
        
        // Calculate the flux
        // WRITE YOUR CODE HERE TO CALCULATE THE FLUX USING THE INPUT FILE
        for (int i = 0; i<fluxNums.length; i++){
            for (int j = 0; j<fluxNums[0].length; j++){
            fluxNums[i][j] = StdIn.readInt();
            }
        }

        for (int x = 0; x<fluxNums.length; x++){
            for (int y = 0; y<fluxNums[0].length; y++){
                flux += fluxNums[x][y];
            }
        }  
